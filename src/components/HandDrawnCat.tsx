...
export function HandDrawnCat({ pose = 0, className = '', drift = false, original = false }: { pose?: number; className?: string; drift?: boolean; original?: boolean }) {
  const cats = original ? assets.originalCats : assets.cats
  const cat = cats[pose % cats.length]
  ...
  return (
    <motion.span className={`cat ${className}`} aria-hidden="true" style={reduce ? undefined : { rotate: tilt }}>
      {!failed && <motion.img
        src={cat.src} alt="" width={cat.width} height={cat.height} onError={() => setFailed(true)}
